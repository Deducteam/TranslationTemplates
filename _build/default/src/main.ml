...
(* Change the de Bruijn indices of the morphism of a term t, so that it can be using in logical relations *)
let morphism2_term t = 
  let rec aux_var w f i = 
    match w with
    | T.App (t, u, l) -> T.mk_App (aux_var t f i) (aux_var u f i) (List.map (fun x -> aux_var x f i) l)
    | Lam (lo, id, None, t) -> T.mk_Lam lo id None (aux_var t (fun x -> f x - 1) (i + 1)) 
    | Lam (lo, id, Some u, t) -> T.mk_Lam lo id (Some (aux_var u f i)) (aux_var t (fun x -> f x - 1) (i + 1)) 
    | Pi (lo, id, a, b) -> T.mk_Pi lo id (aux_var a f i) (aux_var b (fun x -> f x - 1) (i + 1))
    | Kind -> w
    | Type _ -> w
    | DB (lo, id, n) -> 
      (if n >= i then
    ...
    | Const (_, _) -> w
  in aux_var (morphism_term t) (fun n -> n * 2 + 1) 0;;
...
let rec m_term v = 
  ...
  | Kind -> v
  | Type _ -> v
  | DB (lo, id, n) -> T.mk_DB lo id (n * 2 + 1)
